...
class Game:
    ...
    def _call_child(self, state: PartialPublicState) -> PartialPublicState:
        ...
        next_bet_to = state.oppo_bet_to if next_player_id[0] == 1 else state.player_bet_to
        next_bet_to_three = next_bet_to if next_player_id == 2 else 0
        ...
        next_bet_to_one = state.oppo_bet_to_six if next_player_id[0] == 0 else state.oppo_bet_to
        ...
        if last_action == self.ALLIN:
            ...
            if type(new_player) is tuple:
                ...
                if new_player != self.PLAYER2 or state.is_first_action == True:
                    is_first_action = False
    ...
    def _raise_child(
        ...
        bet_size_two = bet_size if next_player_id == 1 else state.player_bet_to
        bet_size_three = bet_size if next_player_id[0] == 2 else 0
        bet_size_four = bet_size if next_player_id[0] == 3 else 0
        bet_size_five = bet_size if next_player_id[0] == 4 else 0
        bet_size_six = bet_size if next_player_id[0] == 5 else 0
        bet_size_one = bet_size + state.oppo_bet_to_six if next_player_id[0] == 1 else state.oppo_bet_to
        last_action = self._action_id2str[state.last_action]
        if last_action == self.ALLIN:
            is_terimnal = True
        else:
        ...
        return PartialPublicState(
            ...
            oppo_bet_to=bet_size_two + state.player_bet_to,
            oppo_bet_to_three=bet_size_three + state.player_bet_to,
            oppo_bet_to_four=bet_size_four + state.oppo_bet_to_three,
            oppo_bet_to_five=bet_size_five + state.oppo_bet_to_four,
            oppo_bet_to_six=bet_size_six + state.oppo_bet_to_five,
            player_id=int(next_player_id[0]),
            #player_id=1 + (state.player_id % self.NUM_PLAYERS) if state.player_id != 5 else 0,
            is_first_action=False,
            is_terimnal=is_terimnal,
    ...
    def _allin_child(self, state: PartialPublicState) -> PartialPublicState:
        ...
        if type(new_player) is tuple:
            ...
            if new_player == 5:
                ...
                next_player_id = 1 + (int(new_player) % self.NUM_PLAYERS),
        bet_size_one = sel.stack_size if next_player_id[0] == 0 else state.oppo_bet_to
        bet_size_two = self.stack_size if next_player_id == 1 else state.player_bet_to
