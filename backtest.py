...
class PartialPublicState:
    def __init__(
            ...
            player_bet_to: int,
            oppo_bet_to: int,
            oppo_bet_to_three: int,
            oppo_bet_to_four: int,
            oppo_bet_to_five: int,
            oppo_bet_to_six: int,
            player_id: int,
        ...
        self._oppo_bet_to_three = oppo_bet_to_three
        self._oppo_bet_to_four = oppo_bet_to_four
        self._oppo_bet_to_five = oppo_bet_to_five
        self._oppo_bet_to_six = oppo_bet_to_six
    ...
    def __eq__(self, state):
        return (
                ...
                and self.oppo_bet_to_three == state.oppo_bet_to_three
                and self.oppo_bet_to_four == state.oppo_bet_to_four
                and self.oppo_bet_to_five == state.oppo_bet_to_five
                and self.oppo_bet_to_six == state.oppo_bet_to_six
    ...
    def pot_size(self) -> int:
        return self._player_bet_to + self._oppo_bet_to + self._oppo_bet_to_three + self.oppo_bet_to_four + self.oppo_bet_to_five + self.oppo_bet_to_six
    ...
    @property
    def oppo_bet_to_three(self) -> int:
        return self._oppo_bet_to_three

    @property
    def oppo_bet_to_four(self) -> int:
        return self._oppo_bet_to_four

    @property
    def oppo_bet_to_five(self) -> int:
        return self._oppo_bet_to_five

    @property
    def oppo_bet_to_six(self) -> int:
        return self._oppo_bet_to_six
    ...
    def __str__(self) -> Text:
        ...
        ret += "oppo_bet_to_three: %d, " % self._oppo_bet_to_three
        ret += "oppo_bet_to_four: %d, " % self._oppo_bet_to_four
        ret += "oppo_bet_to_five: %d, " % self._oppo_bet_to_five
        ret += "oppo_bet_to_six: %d, " % self._oppo_bet_to_six
...
class Game:
    ...
    def act(
        ...
        #)
        print("player", state.player_id)
        assert action in self._action_id2str, (
                "fatal: unknown action id (%d)" % action
        )
        action = self._action_id2str[action]
        allin_size = self.stack_size
        """
        #allin_size = self.stack_size - state.oppo_bet_to
        if state.player_id == self.PLAYER1:
            allin_size = self.stack_size - state.oppo_bet_to_six
        elif state.player_id == self.PLAYER2:
            allin_size = self.stack_size - state.player_bet_to
        elif state.player_id == self.PLAYER3:
            allin_size = self.stack_size - state.oppo_bet_to
        elif state.player_id == self.PLAYER4:
            allin_size = self.stack_size - state.oppo_bet_to_three
        elif state.player_id == self.PLAYER5:
            allin_size = self.stack_size - state.oppo_bet_to_four
        elif state.player_id == self.PLAYER6:
            allin_size = self.stack_size - state.oppo_bet_to_five
        print("allin_size", allin_size)
        """
        assert allin_size >= 0, (
        ...
        # check & call, raise
            ...
            if state.player_id == self.PLAYER1:
                call_size = state.player_bet_to - state.oppo_bet_to_six
            elif state.player_id == self.PLAYER2:
                call_size = state.oppo_bet_to - state.player_bet_to
            elif state.player_id == self.PLAYER3:
                call_size = state.oppo_bet_to_three - state.oppo_bet_to
            elif state.player_id == self.PLAYER4:
                call_size = state.oppo_bet_to_four - state.oppo_bet_to_three
            elif state.player_id == self.PLAYER5:
                call_size = state.oppo_bet_to_five - state.oppo_bet_to_four
            elif state.player_id == self.PLAYER6:
                call_size = state.oppo_bet_to_six - state.oppo_bet_to_five
            assert call_size >= 0, (
    ...
    def _call_child(self, state: PartialPublicState) -> PartialPublicState:
        ...
        if type(new_player) is tuple:
            if new_player[0] == 5:
                next_player_id = 0,
            else:
                next_player_id = 1 + (int(new_player[0]) % self.NUM_PLAYERS),
        else:
            if new_player == 5:
                next_player_id = 0,
            else:
                next_player_id = 1 + (int(new_player) % self.NUM_PLAYERS),
        is_first_action = False
        next_bet_to = state.player_bet_to if next_player_id[0] == 1 else state.oppo_bet_to
        next_bet_to_three = state.oppo_bet_to if next_player_id[0] == 2 else state.oppo_bet_to_three
        next_bet_to_four = state.oppo_bet_to_three if next_player_id[0] == 3 else state.oppo_bet_to_four
        next_bet_to_five = state.oppo_bet_to_four if next_player_id[0] == 4 else state.oppo_bet_to_five
        next_bet_to_six = state.oppo_bet_to_five if next_player_id[0] == 5 else state.oppo_bet_to_six
        next_bet_to_one = state.oppo_bet_to_six if next_player_id[0] == 0 else state.player_bet_to
        ...
        return PartialPublicState(
            ...
            player_bet_to=next_bet_to_one,
            oppo_bet_to=next_bet_to,
            oppo_bet_to_three=next_bet_to_three,
            oppo_bet_to_four=next_bet_to_four,
            oppo_bet_to_five=next_bet_to_five,
            oppo_bet_to_six=next_bet_to_six,
            player_id=int(next_player_id[0]),
    ...
    def _fold_child(self, state: PartialPublicState) -> PartialPublicState:
        ...
        return PartialPublicState(
            ...
            oppo_bet_to_three=state.oppo_bet_to_three,
            oppo_bet_to_four=state.oppo_bet_to_four,
            oppo_bet_to_five=state.oppo_bet_to_five,
            oppo_bet_to_six=state.oppo_bet_to_six,
            player_id=int(next_player_id[0]),
            #player_id=1 + (state.player_id % self.NUM_PLAYERS) if state.player_id != 5 else 0,
            is_first_action=False,
            is_terimnal=True,
            raise_times=0
        )
    ...
    def _raise_child(
        ...
        if type(new_player) is tuple:
            if new_player[0] == 5:
                ...
                next_player_id = 1 + (int(new_player[0]) % self.NUM_PLAYERS),
            #bet_size_two = bet_size if next_player_id == 1 else state.oppo_bet_to
            #bet_size_three = bet_size if next_player_id == 2 else 0
        else:
            if new_player == 5:
                next_player_id = 0,
            else:
                next_player_id = 1 + (int(new_player) % self.NUM_PLAYERS),
        #if next_player_id == 1:
        #    bet_size_two = bet_size
        #if next_player_id == 2:
        #    bet_size_three = bet_size
        bet_size_two = bet_size + state.player_bet_to if next_player_id[0] == 1 else state.oppo_bet_to
        bet_size_three = bet_size + state.oppo_bet_to if next_player_id[0] == 2 else state.oppo_bet_to_three
        bet_size_four = bet_size + state.oppo_bet_to_three if next_player_id[0] == 3 else state.oppo_bet_to_four
        bet_size_five = bet_size + state.oppo_bet_to_four if next_player_id[0] == 4 else state.oppo_bet_to_five
        bet_size_six = bet_size + state.oppo_bet_to_five if next_player_id[0] == 5 else state.oppo_bet_to_six
        bet_size_one = bet_size + state.oppo_bet_to_six if next_player_id[0] == 0 else state.player_bet_to
        last_action = self._action_id2str[state.last_action]
        if last_action == self.ALLIN:
            is_terimnal = True
        else:
        ...
        return PartialPublicState(
            ...
            player_bet_to=bet_size_one,
            oppo_bet_to=bet_size_two,
            oppo_bet_to_three=bet_size_three,
            oppo_bet_to_four=bet_size_four,
            oppo_bet_to_five=bet_size_five,
            oppo_bet_to_six=bet_size_six,
            player_id=int(next_player_id[0]),
            #player_id=1 + (state.player_id % self.NUM_PLAYERS) if state.player_id != 5 else 0,
            is_first_action=False,
            is_terimnal=is_terimnal,
    ...
    def _allin_child(self, state: PartialPublicState) -> PartialPublicState:
        ...
        if type(new_player) is tuple:
            ...
            if new_player == 5:
                ...
                next_player_id = 1 + (int(new_player) % self.NUM_PLAYERS),
        bet_size_one = self.stack_size if next_player_id[0] == 0 else state.player_bet_to
        bet_size_two = self.stack_size if next_player_id[0] == 1 else state.oppo_bet_to
        bet_size_three = self.stack_size if next_player_id[0] == 2 else state.oppo_bet_to_three
        bet_size_four = self.stack_size if next_player_id[0] == 3 else state.oppo_bet_to_four
        bet_size_five = self.stack_size if next_player_id[0] == 4 else state.oppo_bet_to_five
        bet_size_six = self.stack_size if next_player_id[0] == 5 else state.oppo_bet_to_six
        if last_action == self.ALLIN:
    ...
    def get_feasible_actions(self, state: PartialPublicState) -> List[bool]:
        ...
        if state.player_id == self.PLAYER1:
            ...
            call_size = state.oppo_bet_to_three - state.player_bet_to
        elif state.player_id == self.PLAYER4:
            call_size = state.oppo_bet_to_four - state.player_bet_to
        elif state.player_id == self.PLAYER5:
            call_size = state.oppo_bet_to_five - state.player_bet_to
        elif state.player_id == self.PLAYER6:
            call_size = state.oppo_bet_to_six - state.player_bet_to
